...
min.umi = as.numeric(args[7])
min.prop.1umi = as.numeric(as.character(args[8]))
...
#find name of relevant output directory
...
#read in csv file with extracted VDJ barcodes
data1 = read.csv(paste0(out.dir,"/top_bc_per_cell_VDJ_barcodes_",samp.name,"_",
                n.const_v,"_",n.const_j,"_both.csv"), header = T,
         colClasses = c(rep("character",2),rep("numeric",5)))

#get all unique VDJ barcodes
VDJ.uni = unique(data1$Con.seq)

#make data frame for additions and deletions when compared to input sequence
add.del = as.data.frame( matrix(nrow = length(VDJ.uni), ncol = 13) )
names(add.del) = c("V", "insVD", "D", "insDJ", "J", "delV", "delDl", "delDr", 
                   "delJ", "D.fwdorinv", "errV", "errD", "errJ")

#calculate barcode totals per mouse
rownames(add.del) = VDJ.uni
...
#add column to input data frame to say whether barcode is filtered or not according to splitVDJ
splitVDJ.keep = sapply(data1$Con.seq, function(x) {
  if(x %in% rownames(add.del.ok)){
    return(1)
  }else{
    return(0)
  }
})
data1$splitVDJ.keep = splitVDJ.keep

#write distribution of V, D and J per barcode to file (commented out for usage in combination with tagcleanup script)
write.table(add.del, paste(out.dir, "/VDJ_indels_perbc_",samp.name,".csv", sep=""), 
            sep=",", quote=F, row.names=T)

#write full data with filtering information to new file
write.csv(data1, file = paste0(out.dir,"/top_bc_per_cell_VDJ_barcodes_",samp.name,"_",
                               n.const_v,"_",n.const_j,"_both_splitVDJ.csv"), 
          quote = F, row.names = F)
